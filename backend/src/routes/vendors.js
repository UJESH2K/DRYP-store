...
router.post('/apply', async (req, res, next) => {
  try {
    ...
    res.status(201).json({ message: 'Application submitted successfully. We will review your dossier.' });
...
router.put('/applications/:id', protect, async (req, res, next) => {
  try {
    ...
    res.json({ message: `Application ${status} successfully. Email dispatched.` });
  } catch (error) { next(error); }
});
...
router.post('/register', async (req, res, next) => {
  ...
  try {
    ...
    const application = await VendorApplication.findOne({ email, status: 'approved' });
    console.log(application);
    if (!application) {
      return res.status(403).json({ 
        message: 'Forbidden. Your email must be approved by DRYP administration before registering.' 
      });
    }
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    if (user.role !== 'vendor') {
      return res.status(403).json({ 
        message: 'Access denied. This portal is for registered studios only. Please use the mobile app.' 
      });
    }
...
router.get('/admin/directory', protect, async (req, res, next) => {
  try {
    ...
    const vendors = await Vendor.find().populate('owner', 'email isActive createdAt');
    res.json(vendors);
  } catch (error) { next(error); }
});
...
router.put('/admin/suspend/:vendorId', protect, async (req, res, next) => {
  try {
    ...
    if (!vendor || !vendor.owner) {
      return res.status(404).json({ message: 'Studio or associated user account not found.' });
    }
    ...
    const emailSubject = user.isActive ? 'DRYP: Studio Access Restored' : 'DRYP: Studio Access Suspended';
